...
import { patchModule } from './precompiled-wrappers/index.js'
  ...
  implements PGliteInterface, AsyncDisposable
{
  ...
  // Function pointer for the write callback (used with pre-compiled wrappers)
  ...
  // send data to wasm
  // Function pointer for the read callback (used with pre-compiled wrappers)
  ...
  async #init(options: PGliteOptions) {
    ...
    // Patch the module to use pre-compiled WASM wrappers for addFunction
    // This enables Cloudflare Workers compatibility by avoiding runtime WASM generation
    patchModule(this.mod)

    // Set up the write callback using pre-compiled wrapper
    ...
    // Set up the read callback using pre-compiled wrapper
    ...
    // Register the callbacks with the WASM module
  ...
  async close() {
    ...
    try {
      ...
      // Clean up function pointers using pre-compiled wrapper's removeFunction
